...
class BuyProfileModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        account_info = await self.bot.db.fetchone("SELECT * FROM profiles WHERE channel_id = ?", self.channel_id)
        if not account_info:
        ...
        category = await self.bot.db.get_config("buy_profiles_category")
        ...
        account = ProfileObject(*account_info)
        overwrites, role, tos_agreed = await get_default_overwrites(self.bot, interaction.guild.id, interaction.user.id, ticket_type=f"buy-profile")
        
        channel = await category.create_text_channel(name=f"island-{account.number}⭐-buy", overwrites=overwrites)
...
Once the payment has been confirmed, the seller will provide you with the profile.""",
        ...
        initial_message = await channel.send(
            embed=embed, 
            content=f"<@{account.listed_by}>, {interaction.user.mention} is interested in purchasing your profile.", 
...
class Profile(View):
    def __init__(self, bot:Bot, *args, **kwargs):
        ...
        self.bot = bot
        self.children.insert(-1, Dropdown(bot, "profiles"))
    ...
    async def account_owner(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        account = await self.bot.db.fetchone("SELECT * FROM profiles WHERE channel_id = ?", interaction.channel.id)
        if not account:
        ...
        account = ProfileObject(*account)
        await interaction.response.defer(ephemeral=True)

        embed = discord.Embed(
            title="Profile Owner",
            description=f"This Profile is owned by <@{account.listed_by}>",
            color=discord.Color.blue()
        )
    ...
    @button(
        ...
        custom_id="profiles:information",
        style=discord.ButtonStyle.grey,
        row=2,
        emoji="📊"
    )
    ...
    @button(
        label="Buy",
        custom_id="profiles:buy",
        style=discord.ButtonStyle.blurple,
        emoji="💸",
        row=3
    )
    async def buy_profile(self, button: discord.ui.Button, interaction: discord.Interaction):
        await interaction.response.send_modal(BuyProfileModal(self.bot, title="Buy a Profile", channel=interaction.channel.id))

    @button(
        label="Update Stats",
        custom_id="profiles:update",
        style=discord.ButtonStyle.blurple,
        row=3,
        emoji="🔄"
    )
