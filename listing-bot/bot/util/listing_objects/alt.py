...
class BuyAltModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        account_info = await self.bot.db.fetchone("SELECT * FROM alts WHERE channel_id = ?", self.channel_id)
        if not account_info:
        ...
        category = await self.bot.db.get_config("buy_alts_category")
        ...
        account = AltObject(*account_info)
        overwrites, role, tos_agreed = await get_default_overwrites(self.bot, interaction.guild.id, interaction.user.id, ticket_type=f"buy-alt")
        
        channel = await category.create_text_channel(name=f"alt-{account.number}⭐-buy", overwrites=overwrites)
...
Once the payment has been confirmed, the seller will provide you with the account details.""",
        ...
        initial_message = await channel.send(
            embed=embed, 
            content=f"<@{account.listed_by}>, {interaction.user.mention} is interested in purchasing your Alt Account.", 
...
class Alt(View):
    def __init__(self, bot:Bot, *args, **kwargs):
        ...
        self.bot = bot
        self.children.insert(-1, Dropdown(bot, "alts"))
    ...
    async def account_owner(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        account = await self.bot.db.fetchone("SELECT * FROM alts WHERE channel_id = ?", interaction.channel.id)
        if not account:
        ...
        account = AltObject(*account)
        await interaction.response.defer(ephemeral=True)

        embed = discord.Embed(
            title="Alt Account Owner",
            description=f"This Alt Account is owned by <@{account.listed_by}>",
            color=discord.Color.blue()
        )
    ...
    async def buy_alt(self, button: discord.ui.Button, interaction: discord.Interaction):
        await interaction.response.send_modal(BuyAltModal(self.bot, title="Buy an Alt Account", channel=interaction.channel.id))

    @button(
        label="Update Stats",
        custom_id="alts:update",
        style=discord.ButtonStyle.blurple,
        row=3,
        emoji="🔄"
    )
    async def update_info(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        embed = create_embed_alt_listing(profile_data, profile, account.price, convert_payment_methods(self.bot, account.payment_methods), self.bot, f"<@{account.listed_by}>", account.mining, account.farming)
