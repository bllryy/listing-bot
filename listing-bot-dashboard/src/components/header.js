...
const Header = ({ isDarkMode, toggleDarkMode, user, onLogout }) => {
  ...
  const isOnConfig = location.pathname.includes('/config');
            ...
            <div className="flex flex-wrap items-center gap-1 sm:gap-2 mb-2 sm:mb-0">
              <button
                ...
                className={`px-2 sm:px-4 py-2 rounded-lg transition-colors duration-200 text-xs sm:text-sm font-medium ${
                  !isOnListings && !isOnAuth && !isOnConfig
              ...
              <button
                onClick={() => navigate(`/${botName}/config`)}
                className={`px-2 sm:px-4 py-2 rounded-lg transition-colors duration-200 text-xs sm:text-sm font-medium ${
                  isOnConfig 
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="flex items-center gap-2">
                  {user.user_info?.avatar ? (
                    <img
                      src={`https://cdn.discordapp.com/avatars/${user.user_info.id}/${user.user_info.avatar}.png?size=32`}
                      alt="User Avatar"
                  ...
                  <div className="hidden md:block">
                    <p className={`text-sm ${themeStyles.text}`}>
                      {user.user_info?.global_name || user.user_info?.username || 'User'}
                    </p>
                ...
                <button
                  onClick={onLogout}
                  className={`${themeStyles.buttonBg} ${themeStyles.buttonHover} ${themeStyles.text} px-2 sm:px-3 py-2 rounded-lg transition-colors duration-200 text-xs sm:text-sm`}
                >
                  <span className="sm:hidden">📤</span>
